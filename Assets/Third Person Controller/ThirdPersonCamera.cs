...
public class ThirdPersonCamera : MonoBehaviour {
	...
	void Start () {
		...
		// pick up the saved mouse sensitivity from the settings menu
		cameraSpeed = PlayerPrefs.GetFloat("MouseSensitivity", cameraSpeed);
	...
	void LateUpdate () {
		...
		if (Time.timeScale == 0f) return; // don't move the camera while paused
        ...
        if (CameraTarget) {
            ...
            //Camera Zoom
            desireDistance -= Input.GetAxis("Mouse ScrollWheel") * 6f;
